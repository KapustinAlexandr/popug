...
public class AuthEventsConsumer : KafkaConsumer
{
    ...
    public override async Task OnMessage(Message<string, string> message)
    {
        ...
        if (data?.Type == "REGISTER")
        {
            ...
            await db.Users.InsertOrUpdateAsync(
                () => new UserEntity 
                {
                    UserId = data.UserId,
                    UserName =
    ...
    protected override ConsumerConfig OnConfigCreate(ConsumerConfig config)
    {
        config.AutoOffsetReset = AutoOffsetReset.Earliest;
        return base.OnConfigCreate(config);
    }
}